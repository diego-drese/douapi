...
class ProcessZipDou extends Command {
	 ...
	 * @var string
	 */
	protected $signature = 'DouApi:ProcessZipDou';
	...
	public function getPathZip($file=''){
		if(!is_dir(storage_path('douapi/'.$this->folderZip))){
			File::makeDirectory(storage_path('app/'.$this->folderZip));
		}
		return storage_path('douapi/'.$this->folderZip.$file);
	}
	...
	public function getPathXml($file=''){
		if(!is_dir(storage_path('douapi/'.$this->folderXml))){
			File::makeDirectory(storage_path('app/'.$this->folderXml));
		}
		return storage_path('douapi/'.$this->folderXml.$file);
	}
	...
	public function unzipFiles(){
		...
		foreach ($files as $file){
			...
			rename($this->getPathZip($file), $this->getPathZipAttachment($file));
