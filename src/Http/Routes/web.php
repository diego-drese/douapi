...
Route::group(['prefix' => $prefix_url, 'middleware' => ['web', 'auth', 'Oka6\Admin\Http\Middleware\MiddlewareAdmin']], function () {
	Route::get('/dou', 'Oka6\DouApi\Http\Controllers\DouController@index')->name('douapi.index')->where(['iconAdmin' => 'mdi mdi-book-open-page-variant', 'menuAdmin' => "DOU", 'parentRouteNameAdmin' => 'Dou', 'nameAdmin' => 'DOU', 'isDefaultAdmin' => '1']);
	
	/*Plans admin */
	Route::get('/plans', 'Oka6\DouApi\Http\Controllers\PlanController@index')->name('admin.plan.index')->where(['iconAdmin' => 'fas fa-money-bill-alt', 'menuAdmin' => "Plans", 'parentRouteNameAdmin' => 'System Admin', 'nameAdmin' => 'Plans Listing', 'isDefaultAdmin' => '1']);
	Route::get('/plans/create', 'Oka6\DouApi\Http\Controllers\PlanController@create')->name('admin.plan.create')->where(['iconAdmin' => 'fas fa-plus-square', 'parentRouteNameAdmin' => 'douapi.plan.index', 'nameAdmin' => 'Plans Create',]);
	Route::post('/plans', 'Oka6\DouApi\Http\Controllers\PlanController@store')->name('admin.plan.store')->where(['iconAdmin' => 'fas fa-floppy-o', 'nameAdmin' => 'Save Plans']);
	Route::get('/plans/{id}', 'Oka6\DouApi\Http\Controllers\PlanController@edit')->name('admin.plan.edit')->where(['iconAdmin' => 'fas fa-edit', 'parentRouteNameAdmin' => 'douapi.plan.index', 'nameAdmin' => 'Plans Edit']);
	Route::post('/plans/{id}', 'Oka6\DouApi\Http\Controllers\PlanController@update')->name('admin.plan.update')->where(['iconAdmin' => 'fas fa-edit', 'parentRouteNameAdmin' => 'admin.plan.index', 'nameAdmin' => 'Plans Update']);
	...
	Route::get('/read-pdf/{file}', 'Oka6\DouApi\Http\Controllers\SubscriptionController@readPdf')->name('douapi.subscription.read.pdf')->where(['iconAdmin' => 'fas fa-edit', 'parentRouteNameAdmin' => 'douapi.subscription.index', 'nameAdmin' => 'Subscription read pdf']);
